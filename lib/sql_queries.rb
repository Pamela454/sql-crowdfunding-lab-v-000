...
"SELECT projects.title, SUM((pledges.amount) - projects.funding_goal) AS amount_left FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING pledges.amount >= projects.funding_goal"
end
